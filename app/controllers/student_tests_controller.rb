class StudentTestsController < ApplicationController
  before_action :confirm_log_in, only: [:new, :results]
 # before_action :verify_student, only: [:new, :results]
  ...
  def new   
    ...
    # We extract the newly created check_authorisation, only: [:new, :results]answer object's id and combine it with question id add student_test_id
  ...
  def create 
    ...
    # .fill_in_student_test joins the student test (created in the new action) with question_answer obejects by 
    # storing student_test_id, question_id and answer_id 
    # answers are also created inside the function
    StudentTest::SolveTest.fill_in_student_test(student_test_params)
    StudentTest::CheckTest.check(@student_test, @question_answers)
    redirect_to action: :results, student_test_id: @student_test.id
  end 

  def results
    @test_with_question_answers = StudentTest.find(params[:student_test_id])
...
=begin   
  def verify_student
    check_authorisation(set_student)
  end 
=end   
end
